...
def parse_api_response(api_data: dict) -> List[Dict[str, any]]:
    ...
    try:
        ...
        # ⚠️ 중요: API가 제공하는 리스트 순서 자체가 순위임
        # viewCount나 starScore로 재정렬하면 오히려 잘못된 순위가 됨
        # (viewCount는 대부분 0이고, starScore로도 정렬되어 있지 않음)
        global_rank = 1
...
def extract_webtoon_from_api_item(item: dict, rank: int, weekday: Optional[str] = None, weekday_rank: Optional[int] = None) -> Optional[Dict[str, any]]:
    """
